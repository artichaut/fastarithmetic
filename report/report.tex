...
\emph{just-in-time} (jit) compilation. This means that when you write a
function, e.g. 
\begin{verbatim}
...
quadratic in
the degree of the extension you are dealing with. The algorithms presented
...
performing a matrix-vector product $v\mapsto Mv$, you can \emph{transpose} it to
...
principle also changes the basis you are working with.
...
discussed in~\cite{DeDoSc14}, that's why we will not show all the pseudo-codes but
...
at
\url{https://gitbub.com/edouardRousseau/FastArithmetic.jl}. What's more, it is really
...
\end{verbatim}
You may want to update the package, because the code is very likely to 
change, so just use
\begin{verbatim}
julia> Pkg.update()
\end{verbatim}
...
\texttt{@benchmark} macro available in the BenchmarkTools package. This macro lets you
...
there is still a bit of instability.
...
certain values of $m$ tested (I also could have change the script\dots).
...
implementation and difficulties for Julia to handle matrix operations 
appearing in \textbf{Phi2}. \textbf{inversePhi2} is even slower, but it is not 
surprising, since it uses \textbf{mulmodT} a lot of times, which is very slow 
...
Unfortunately, the power of Julia has not been demonstrated in this paper, and 
we were unable to match C speed. But, as we saw, certain algorithms were not so 
far. With further investigations, we could understand why some functions are 
slow; and, since the functions are inter-dependent, speed up the whole. If you 
find any error in this report, you can send me a message 
(edouard.rousseau@u-psud.fr). You can do the same if you find the code in 
FastArithmetic.jl unreadable, or if you see how I managed to slow the 
algorithms. Finally, I would like to thank \textsc{Luca De Feo} for taking me 
as an intern during the summer, and for answering my naive questions by mail or 
in front of a coffee. I also hope you enjoyed reading this paper, have a good 
day!
